...
> **定位：买卖是手段，结构是目的。**
> Star.org 不卖星星，也不只是一个纪念品店——它想为「人类记录了什么」建一层可验证、可长期引用、公开可查的结构。
> 认领一颗星是这层结构上的第一条记录，也是它目前唯一的资金来源；**商品形态与合规框架都不变**。
> 三条边界：① **我们不掌握信息，只做登记与溯源**——是登记方，不是所有者；② **无主的信息可以承载，有主的信息须经同意**——星空与物理地球的信息无主；心智的信息有主，只能由本人或经其同意登记（见理念文档 §4.4）；③ **商品是纪念品，结构是公共品**——付费得到的是纪念性认领记录；认领表、数据格式与白皮书公开、免登录、可自由引用。
> 完整论述见 `docs/Star.org-核心理念.md` §8。
