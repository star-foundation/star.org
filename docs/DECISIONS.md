...
- 文档：4.2 用 Actions `concurrency` 分组串行化登记流程。
...
- 可配置：`site.config.json → policy.anonymousNameInCertificate = false` 时，
  匿名登记的证书本身也不印姓名（显示"匿名登记人"），适合对匿名要求更严格时使用。

## D5. 隐私字段的机器化校验

- 文档：TC-REG-03 要求"登记记录中不包含邮箱、订单号等隐私信息"。
