...
export function deterministicSlug(orderId, secret, length = 10) {
  if (!orderId) return randomSlug(length);
  if (!secret) return randomSlug(length);
  const digest = crypto.createHmac('sha256', secret).update(String(orderId)).digest();
  return base32(digest, length);
}
